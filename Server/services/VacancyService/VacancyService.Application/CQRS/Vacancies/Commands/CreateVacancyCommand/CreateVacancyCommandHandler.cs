...
namespace VacancyService.Application.CQRS.Vacancies.Commands.CreateVacancyCommand
{
    internal sealed class CreateVacancyCommandHandler : IRequestHandler<CreateVacancyCommand, VacancyDto>
    {
        ...
        {
            var entity = _mapper.Map<Vacancy>(request.Vacancy);
...
#pragma warning restore CS8601 
        ...
        private async Task<Vacancy> HandleLanguagesAsync(Vacancy vacancy, CancellationToken cancellationToken = default)
        {
            if (vacancy.Languages != null)
            {
                ...
                //    ((List<Language>)vacancy.Languages)
                //        .Add(await _repositoryManager.Language
                //        .GetLanguageByIdAsync(language.Id, true, cancellationToken)
                //        ?? throw new LanguageNotFoundException(language.Id));

                //    var Levels = language.Levels.ToList();
                //    language.Levels = new List<Level>();

                //    foreach (var level in Levels)
                //    {
                //        ((List<Level>)language.Levels)
                //             .Add(await _repositoryManager.Level
                //             .GetLevelByIdAsync(level.Id, true, cancellationToken)
                //             ?? throw new LevelNotFoundException(level.Id));
                //    }

                //    ((List<Language>)vacancy.Languages)[i].Levels = new List<Level>();

                //    ((List<Level>)((List<Language>)vacancy.Languages)[i].Levels).AddRange(Levels);
                //}
            }
