...
namespace VacancyService.Application.CQRS.Vacancies.Queries.GetVacancyCollection
{
        ...
        IRequestHandler<GetVacancyCollectionQuery, SearchResultInfo<VacancyDto>>
    {
        ...
        {
            var vacancies = await _repositoryManager.Vacancy.GetAllAsync(request.VacancyParameters, request.TrackChanges, cancellationToken);

            var vacanciesDto = _mapper.Map<IEnumerable<VacancyDto>>(vacancies);

            var count = await _repositoryManager.Vacancy.CountAsync(request.VacancyParameters, cancellationToken);
